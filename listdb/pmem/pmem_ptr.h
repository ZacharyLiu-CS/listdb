...
public:
  ...
  PmemPtr(const int16_t pool_id, const uint64_t offset)
      : data_(Encode(pool_id, offset)) {}

  PmemPtr(int16_t pool_id, char *vaddr) {
    uint64_t offset =
        (uintptr_t)vaddr - (uintptr_t)Pmem::pool(pool_id).handle();
    data_ = Encode(pool_id, offset);
  }

  PmemPtr(const uint64_t dump) : data_(dump) {}
  ...
  template <typename T> static T *Decode(const uint64_t dump);

  template <typename T> static T *Compose(int16_t pool_id, uint64_t offset) {
    if (offset == 0) {
      return nullptr;
    }
...
template <typename T> inline T *PmemPtr::get() { return (T *)get(); }

inline uint64_t PmemPtr::dump() { return data_; }
...
template <typename T> inline T *PmemPtr::Decode(const uint64_t dump) {
  if (dump == 0) {
    return nullptr;
  }
  // int *p2 = (int *)(((uintptr_t)p1 & ((1ull << 48) - 1)) |
